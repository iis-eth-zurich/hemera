...
extern char __omp_64bit;
int is_omp_64bit() {
  return (int)&__omp_64bit == 1;
}

typedef kmp_int64 _kmp_ptr64;
typedef kmp_int32 _kmp_ptr32;
...
static void __microtask_wrapper(void *arg) 
{
  ...
  if(is_omp_64bit()) {
    _kmp_ptr64 *args = (_kmp_ptr64*) arg;
    __task_type64 fn = (__task_type64) args[0];
    _kmp_ptr64 id_addr = (_kmp_ptr64)((uint32_t) &id);
    _kmp_ptr64 arg_addr = (_kmp_ptr64)(uint32_t) &args[1];
...
void __kmpc_fork_call(ident_t *loc, kmp_int32 argc, kmpc_micro microtask, ...)
{
  ...
  int arg_size = 0;
  if(is_omp_64bit()) {
    arg_size = (argc+1)*sizeof(_kmp_ptr64);
  } else {
  ...
  if(is_omp_64bit()) {
    ...
    ((_kmp_ptr64*)args)[0] = (_kmp_ptr64) microtask;
  }
  va_start(ap, microtask);
  for(int i=1; i<=argc; ++i) {
    if(is_omp_64bit()) {
      ((_kmp_ptr64*)args)[i] = (_kmp_ptr32) va_arg(ap, _kmp_ptr64);
    } else {
